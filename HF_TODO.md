...
已完成首批 M5/16GB 证据:0.1B load/forward/generate、0.4B fp32/fp16 load/forward/短 generate、0.4B fp32/fp16 prompt 16/64/128 generation sweep、tiny native train、tiny PEFT/Trainer、0.1B 和 0.4B 真模型 PEFT LoRA backward + HF Trainer + TRL SFT/DPO/GRPO、0.4B Trainer/TRL 2-step、1.5B fp16 load/forward/短 generate + prompt16 sweep、1.5B fp32 PEFT LoRA manual backward。继续补:

- 扩展 0.4B Apple 到 256/512+ prompt、更长 decode 和 3+ step 稳定性行;
- 把 1.5B 从首批 fp16 inference / fp32 manual PEFT 行扩到 Trainer、TRL SFT/DPO/GRPO、长 prompt/decode sweep,并记录 memory-pressure;
