...
- 新增 MLX lazy graph prefill eval batching seam 与 `scripts/mlx_prefill_eval_interval_bench.py`;模型默认仍为 interval=1,Apple acceptance 默认 interval=2。M5 512-char、4-repeat、顺序轮换实测(`bench/results_mlx_prefill_eval_m5_20260710_{fp16,w4}.jsonl`)中,0.1B/0.4B/1.5B fp16 median 提升≈1.05x/1.28x/1.09x,0.4B/1.5B W4≈1.38x/1.32x,且 logits、完整 state/cache、seen tokens、next token 全部 max_abs=0。新的同机 eval2 gate(`bench/results_qwen35_apple_m5_20260710_eval2_{fp16,w4}.jsonl`)仍只有 fp16 prefill≈0.120x/0.050x Qwen、W4≈0.088x/0.042x,因此这只是 host-sync 优化;主线必须继续做 MLX/Metal 的 DPLR/WY chunk summary→prefix combine→chunk apply/output。
- Apple DPLR/WY Stage 1 已落:`rwkv7_hf/mlx_dplr_prefill.py` 提供 compact summary→prefix combine→chunk apply/output 的 MLX oracle,并为 summary 与 apply/output 提供首版 custom Metal kernel;`scripts/mlx_dplr_prefill_bench.py` 做分阶段 parity/speed gate。M5 production-shaped synthetic `B1/T512/H16/N64/C64/fp16`,warmup2/repeat5 中,全 Metal summary + MLX prefix + Metal apply median≈60.249ms/8,498.11 effective tok/s,比 MLX recurrent oracle≈1.28x、比 high-level three-stage≈3.44x;factor diff≤3.73e-08,output/final-state diff≤1.53e-04/1.14e-04。原始行:`bench/results_mlx_dplr_stage1_target_m5_20260710.jsonl`。summary≈58.801ms,仍是绝对瓶颈;待并行分块、partial chunk、layer-major 真模型集成与 0.4B Qwen gate,当前不可把 synthetic 结果写成模型级胜出。
- Apple DPLR 真模型 Stage 2 已接通:MLX prefill 可选 layer-major sequence 路径,vectorized shift-mix/projection/FFN + Metal summary/apply,partial chunk 用 identity/no-op padding;默认仍 recurrent,`auto` 只在 token数≥128 时启用。M5 prompt512/111 tokens 实测 fp16 median recurrent→DPLR:0.1B `262.04→408.63`(1.56x),0.4B `117.91→175.83`(1.49x),1.5B `37.57→123.45`(3.29x);0.4B W4 `78.29→115.40`(1.47x)。全部保持 8-token continuation exact,logits/state max_abs≤0.0625/0.128;但 peak 增加约8%-24%,所以未默认启用。0.4B 同 prompt Qwen rows 也保持 fp16/W4 的 recurrent 32-token continuation exact,但 prefill/TTFT gate 仍失败。证据:`bench/results_mlx_dplr_model_m5_20260710_{fp16,w4}.jsonl`、`bench/results_qwen35_apple_m5_20260710_dplr_auto_{fp16,w4}.jsonl`。下一步是 summary tile/parallel、长 prompt、decode/peak 回收、1.5B 质量和跨 M 系列验证。
- Apple DPLR Stage 3 已把 summary 改成每 `(B,chunk,head)` 一个 threadgroup、每 head-dim row 一个 lane;旧 scalar Metal 作为独立 oracle。M5 synthetic T512/H16/N64/C64 中 scalar→tiled summary `54.801→3.399ms`(16.12x),五组 factor bit-identical,完整三阶段≈3.421ms。最终 opt-in/auto 调度为 chunk64、tiled、阈值8 token、T≥64 每4层 materialize、512-token 外层窗口;总 backend 默认仍 recurrent。M5 prompt512/111-token fp16 median recurrent→DPLR:0.1B `257.42→5058.87`(19.65x),0.4B `118.62→2143.54`(18.07x),1.5B `37.56→1006.19`(26.79x),peak ratio≈1.161x/1.090x/1.053x,全部8-token exact。0.4B 223/888/1774-token 长上下文继续 exact;窗口把后两档 peak 控制在≈1.16-1.17GB。Qwen3.5 0.8B chars512 首次出现 fp16 conservative decode/prefill/TTFT 全 pass 的单次矩阵,W4 prefill/TTFT pass但decode≈0.950x;chars128/cold compile与跨轮稳定性仍失败,下一主线改为 Apple decode fusion + cold compile + M1-M4/M5 Pro。证据:`bench/results_mlx_dplr_tiled_{stage,model,long}_m5_20260710*.jsonl`、`bench/results_qwen35_apple_m5_20260710_dplr_tiled_final_{fp16,w4}.jsonl`。
- Apple guarded compiled decode 已落:MLX 全模型单 token graph 显式 compile/warmup,flatten 后的完整 recurrent/cache state 作为纯函数输入输出;`auto` 只有在具体 model+batch 的 eager/compiled 多步 greedy、最终 logits、全部 state 严格一致后才提升,否则回退 eager。M5 0.4B B1 prompt512/decode64 中 fp16 `96.39→119.99 tok/s`(1.245x),W4 `97.79→119.46`(1.222x),两者64 token/logits/state exact,peak 仅约增加0.6%-0.7%。0.1B 因64步数值漂移、1.5B 因第17步 token mismatch 被 guard 拒绝并安全回退。配合 tiled DPLR 后,0.4B fp16 对 Qwen3.5 0.8B 的 chars128/512 conservative decode/prefill/TTFT 速度门全部通过:ratio=`1.136/1.035/0.473` 与 `1.157/1.687/0.461`(TTFT 越低越好);总状态仍 unknown,因为 Ollama 只有 loaded memory 而无可比 peak。下一步:修复0.1B/1.5B exact promotion、W4 decode projection/dequant fusion、真实 peak-to-peak、M1-M4/M5 Pro/Max。证据:`bench/results_mlx_decode_compile_m5_20260710_{fp16,w4}.jsonl`、`bench/results_qwen35_apple_m5_20260710_dplr_tiled_compiled_fp16.jsonl`。
- Apple fast-LayerNorm parity 路线已补:`RWKV7_MLX_DECODE_NORM_BACKEND=fast` 只把 decode 的三个标准 LayerNorm 边界换成 MLX fast primitive,GroupNorm 与 prefill 保持 reference。compiled cache 随 norm backend 失效;提升前同时要求 active eager/compiled 严格 exact,以及 reference-norm 64-token exact greedy + 默认 logits/state≤0.25/0.5。M5 fp16 fast eager→compiled:0.1B `203.47→255.01`(1.253x),0.4B `88.09→101.02`(1.147x),1.5B `30.72→33.64`(1.095x),reference tokens 全 exact,reference diff 最大0.0625/0.1875。但策略不能一刀切:0.1B 对旧 reference eager 仍约1.16x,0.4B 应继续用更快的 reference compiled `119.99`,1.5B 应继续 reference eager≈36.5。W4 0.1B `176.06→227.22`(1.291x);1.5B W4 因 compiled logits diff=0.015625 且总体仍慢于 fp16 保持 fallback。证据:`bench/results_mlx_decode_fast_layernorm_m5_20260710_{fp16,w4}.jsonl`。下一步是把这种 model/device 选择落成静态策略表,再做 batch>1 与跨 M 系列验证。
