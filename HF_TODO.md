...
- `scripts/export_rwkv7_coreml.py` 已提供 import-safe CoreML manifest / full-logits export 原型,`bench/run_coreml_apple_baseline.py` 已提供 CoreML runtime plan/skip/partial row 入口,但 stateful decode/prefill、CoreML state correctness 和 ANE tok/s 行未完成;在此之前,移动端“超过”只能算 MLX/Mac 侧阶段证据。
