...
| 生产性能 | 部分 | V100 fast-token/native-graph 已加入 norm/mix、sm70 projection/FFN 与 raw recurrent-output 深融合;0.1B/0.4B/1.5B × bsz1/2/4/8 decode 同卡比率 `0.629x-1.185x` Albatross。sm70 split-row native prefill 多数达到 P2。4090 fixed-shape prefill graph 已使 0.4B prompt512 bsz1 达到 `1.011x` Albatross，bsz4=`0.870x`;W8/W4 speed lane 同时实现 footprint 下降且 prefill/decode 不慢于 w16。5090 native speed lane 已有 1.5B/2.9B/7.2B full matrix。剩余为 4090 大 batch、更多卡/模型与 full-memory quant prefill fused kernel。 |
...
| RTX 4090 / Ada | **B1 prefill 与量化 speed lane 达标，B4 继续** | exact-4090 默认 fixed-shape prefill CUDA Graph + split scan/state-prep/output/shift:0.4B fp16 prompt512 bsz1/4=`60.7k/102.4k tok/s`，为 Albatross 的 `1.011x/0.870x`;1.5B bsz1=`32.4k tok/s`，logits/greedy/cache handoff 通过。W8 speed lane payload `0.926x`、prefill `1.011x` fp16、decode bsz1/2/4/8=`1.001x-1.020x`;W4 speed lane payload `0.891x`、prefill `1.010x` bf16、decode 已测 bsz1/4=`1.043x/1.058x`。W4 memory lane仍保留 payload `0.399x` 与 bsz1/2/4/8 超 Albatross decode。剩余重点是 B4 prefill 与 full-memory quant prefill。见 BENCHMARK 4090 段。 |
