...
| 官方权重转换 | 已完成 | 官方 `.pth` → HF `safetensors`;shape 推断覆盖已发布尺寸。新增 mmap + meta-template `--low-memory` 路径，已在 48GB/no-swap 5090 主机完成 13.3B 六分片转换和加载。 |
...
| 量化加载 | 大模型 V100 / A6000 功能通过;5090 native speed lane 压力缺口已闭合 | bnb 8/4-bit 加载/生成、显存下降;0.4B/1.5B/2.9B/7.2B V100 与 RTX A6000 pass/pass。RTX 5090 新 batched MM8/MM4 kernel 的 36-row bsz8 压力矩阵全部 footprint 下降且 same-next 24/24；2.9B/7.2B 全部在 1% fp16 等价带，合并矩阵在 2% 等价带通过。13.3B speed-policy W8/W4 边界也已跑通。`memory` policy 仍是最大 footprint 下降但非速度达标 lane。 |
...
| Apple Silicon / MPS | **M5 production-close 通过** | MLX groupwise W4 + tiled DPLR + compiled/speculative decode 已通过同机 chars512/decode64 的 0.4B vs Qwen3.5 0.8B、1.5B vs Qwen3.5 2B 保守 decode/prefill/TTFT/peak-memory 门禁；三轮 speculative 输出通过 target-greedy oracle。CoreML stateful 0.1B/0.4B state transfer、chunk split、HF greedy 已通过，INT4 质量与 ANE occupancy 仍开放。见 [`docs/hardware/APPLE_PRODUCTION_CLOSE.md`](docs/hardware/APPLE_PRODUCTION_CLOSE.md)。 |
| 生产性能 | 部分 | V100 fast-token/native-graph 已完成 exact-sm70 production-close:0.1B/0.4B/1.5B × bsz1/2/4/8 dense decode=`0.908x-1.248x` Albatross，prompt512 prefill=`0.930x-1.047x`;native W8/W4 speed lane payload=`0.803x-0.956x`、decode=`1.006x-1.128x` fp16、paired prefill=`0.996x-1.007x` fp16，正确性/cache handoff 全通过。4090 fixed-shape prefill graph 的 0.4B prompt512 bsz1/4 已达 `64.51k/107.87k tok/s`，B4 对同机当次 Albatross=`1.007x`，对历史最强记录=`0.916x`;W8/W4 speed lane 同时实现 footprint 下降且 prefill/decode 不慢于 w16。5090 native speed lane 已有 1.5B/2.9B/7.2B full matrix；0.4B MATH500 `500x64` 达 pass@64=`0.38`，对提交内 Albatross reference 的 summary/decode speed gate=`4.336x/4.871x`。剩余为更多卡/模型、同卡 Albatross 复跑、历史高水位追赶与 full-memory quant prefill fused kernel。 |
...
| RTX 50 系 / Blackwell | **5090 quant/13.3B/MATH500 production-close 证据已补** | 历史 216-row 功能矩阵之上，新 batched MM8/MM4 kernel 把 7.2B prompt2048/decode512/bsz8 从 `0.7619x/0.6695x` 提升到 paired `0.9913x/0.9919x`；2.9B/7.2B 压力行严格 1% 等价门通过。`--low-memory` 已在 48GB/no-swap 主机转换 13.3B，load/forward/generate peak `25536.6MiB` 通过。0.4B full MATH500 avg@64 达 pass@64=`0.38`，对提交内 Albatross reference 的 summary/decode speed=`4.336x/4.871x`；该项不是同卡实时 Albatross 复跑。见 [`bench/5090_blackwell_production_close_20260712/`](bench/5090_blackwell_production_close_20260712/README.md)。 |
...
| Apple Silicon / MPS | **M5 MLX production-close；CoreML stateful correctness pass** | M5 的 0.4B/1.5B Qwen3.5 速度、TTFT、显存门禁已闭合；继续补 M1-M4/Pro/Max、CoreML INT4 和 ANE placement。 |
...
- 量化速度分 lane: bnb W8/W4 仍慢于 fp16;native mm8/mm4 `speed` policy 的 RTX 5090 7.2B 大压力缺口已闭合到 1% paired 等价带。剩余重点是更大 footprint 下降的 `memory` policy、1.5B W8 个别 1%-2% 波动行与跨卡复现。
...
- 更多卡覆盖(Turing / H100 / AMD / 更多 50 系)与更长训练吞吐。
