...
| 量化加载 | 大模型 V100 / A6000 功能通过;5090 native speed lane 已补 | bnb 8/4-bit 加载/生成、显存下降;0.4B/1.5B/2.9B/7.2B V100 与 RTX A6000 pass/pass;RTX A6000 另有 native mm8/mm4 decode telemetry 行。RTX 5090 native MM8/MM4 `speed` policy 已补 1.5B/2.9B/7.2B footprint 下降、decode≈0.97x-1.01x fp16、logits/greedy 对齐证据;`memory` policy 仍是最大 footprint 下降但非速度达标 lane。 |
...
| 生产性能 | 部分 | V100 fast-token/native-graph 提升 decode;Albatross 级未闭合。量化改为 speed/memory 双 lane:5090 native speed lane 在 1.5B/2.9B/7.2B 接近闭合,full-memory quant 仍需 fused quant kernels。 |
...
- 量化速度分 lane: bnb W8/W4 仍慢于 fp16;native mm8/mm4 `speed` policy 的 `lm_head` 行在 5090 1.5B/2.9B/7.2B 已有 footprint 下降、logits 对齐、decode≈0.97x-1.01x fp16 证据;更大 footprint 下降的 `memory` policy 仍需 fused/native 量化矩阵。
