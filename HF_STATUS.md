...
| 生产性能 | 部分 | V100 fast-token/native-graph 提升 decode;sm70 split-row native prefill 已把 0.1B prompt512 bsz1/2/4/8 提升到 Albatross 的 0.815x/0.793x/0.863x/0.796x,0.4B/1.5B 大部分行达到 P2,但 Albatross parity 仍未闭合。量化改为 speed/memory 双 lane:5090 native speed lane 已有 1.5B/2.9B/7.2B full matrix,正确性与显存目标成立,速度多形状超过 fp16,但 7.2B 大 bsz/长 prompt 压力行仍需 fused/head-dispatch 优化;full-memory quant 仍需 fused quant kernels。 |
