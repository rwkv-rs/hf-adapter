...
| 生产性能 | 部分 | V100 fast-token/native-graph 已加入 norm/mix、sm70 projection/FFN 与 raw recurrent-output 深融合;0.1B/0.4B/1.5B × bsz1/2/4/8 decode 同卡比率 `0.629x-1.185x` Albatross,12/12 通过 P1,bsz8 全部 P3,其中 0.4B/1.5B 超过 Albatross。sm70 split-row native prefill 把 0.1B prompt512 bsz1/2/4/8 提升到 Albatross 的 0.815x/0.793x/0.863x/0.796x,0.4B/1.5B 大部分行达到 P2。量化改为 speed/memory 双 lane:5090 native speed lane 已有 1.5B/2.9B/7.2B full matrix,正确性与显存目标成立,速度多形状超过 fp16,但 7.2B 大 bsz/长 prompt 压力行仍需 fused/head-dispatch 优化;full-memory quant 仍需 fused quant kernels。 |
...
| RTX 4090 / Ada | **W4 decode 达标，W8/prefill 继续** | 0.4B TorchAO group-128 W4 + native-graph + bf16 Ada W/A/G/V fusion 在 bsz1/2/4/8 达到 `927/1713/3093/3407 tok/s`，为同口径 bf16 的 `1.24x-1.62x`、Albatross fp16 的 `1.17x-1.52x`;payload `342.8/859.8 MB=0.399x`，cosine `>=0.999239`，next-token 全匹配。1.5B bsz1/2 进一步达到 bf16 的 `2.17x/2.37x`、payload `0.355x`。W8 与 prefill 尚未达标。见 BENCHMARK 4090 段。 |
...
- Albatross / RWKV-LM 的 V100 decode P1 已闭合;仍需统一 P2/P3、更多模型与跨卡复现(见 [`docs/performance/FUSED_BACKEND.md`](docs/performance/FUSED_BACKEND.md))。
