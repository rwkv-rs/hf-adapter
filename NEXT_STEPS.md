...
- quant：已新增 bitsandbytes 8bit/4bit smoke 和 benchmark，并让量化 cached decode 通过 `RWKV7_FAST_FORWARD_QUANT=1` 默认走 HF fast-forward 的 FLA fallback。V100 0.1B 上 model footprint 从 fp16 `364.4MB` 降到 8bit `278.4MB`、4bit `235.3MB`；短 benchmark 中 generic bnb reference decode `7.9` / `22.5 tok/s`，fast-forward 后到 `8.4` / `27.1 tok/s`，greedy next token 保持一致；但仍低于 fp16 native-graph `251.1 tok/s`，所以仍需自定义/融合量化 serving path 才能满足“不比 16bit 慢”的目标。
...
5. 性能路径：
   ...
   - 已新增 HF-only speculative decoding helper 和真实小模 draft benchmark：`rwkv7_speculative_generate()` 用 RWKV/HF draft model 提案，target model 以 block forward 验证并输出 acceptance telemetry；V100 0.1B draft -> 0.4B target 已匹配 target greedy 8/8 new tokens，acceptance 7/9=0.778；下一步优化 draft 选择、verify/resync overhead 和速度
