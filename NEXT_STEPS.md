...
- speed：`fuse_norm=false` + `RWKV7StateCache` 下标准 remote-code HF decode 约 41.2 tok/s；FLA `rwkv7_forward_token` V100 bsz=1 约 59.2 tok/s；opt-in `RWKV7_FAST_TOKEN_BACKEND=native_jit` 的 HF fast-token bsz=1 已到 92.1 tok/s，和 official 92.1 tok/s 持平，target gate 已通过。bsz=1/2/4/8 batch sweep 中 native_jit 对 bsz=1 生效（91.9 tok/s），bsz>1 暂时 fallback 到 FLA（117.7/233.3/465.5 aggregate tok/s）；dynamic batch 从 205.2 提升到 345.7 total tok/s；component bench 显示 `attn_linears_lora` 最大，约 9.87ms/token。
...
5. 性能路径：
   ...
   - 已新增 batch cache/sweep、dynamic-batch reorder/drop harness、decode microbench、decode component bench、projection/LoRA bench、gap analyzer 和 result gate；V100 bundle 已跑通，bsz=1 fast-token native_jit 已到 official parity，下一轮重点是把 native_jit/graph 思路扩展到 batched fast-token 和 dynamic batching，优先减少 tiny kernel launch / Python dispatch
   - native JIT block-step 已接入 `rwkv7_forward_token` 的 bsz=1 opt-in backend；下一步把同样的 block-step packing / graph-capture 思路扩展到 bsz>1 和 dynamic reorder/drop serving 语义
