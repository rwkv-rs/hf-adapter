...
// Variant of the warp-pipelined row-block schedule that stores the shared
// per-token vectors in fp16 instead of fp32.  This is an opt-in probe for
// shared-memory bandwidth/footprint; the row state and reductions remain fp32.
template <typename scalar_t, int ROWS_PER_BLOCK, bool W_PRECOMPUTED=false>
__global__ void rwkv7_state_scan_prep_n64_rowblock_warp_pipelined_half_shared_kernel(
    const scalar_t* __restrict__ r,
    const scalar_t* __restrict__ w_raw,
    const scalar_t* __restrict__ k_raw,
    const scalar_t* __restrict__ v_raw,
  ...
  __shared__ scalar_t r_s[2][N];
  __shared__ scalar_t w_s[2][N];
  __shared__ scalar_t k_s[2][N];
  __shared__ scalar_t a_s[2][N];
  __shared__ scalar_t kk_s[2][N];
  __shared__ scalar_t v_row_s[2][ROWS_PER_BLOCK];
...
template <typename scalar_t, int ROWS_PER_BLOCK>
__global__ void rwkv7_state_scan_prep_n64_rowblock_warp_pair_kernel(
    const scalar_t* __restrict__ r,
    const scalar_t* __restrict__ w_raw,
    const scalar_t* __restrict__ k_raw,
    const scalar_t* __restrict__ v_raw,
  ...
  constexpr int N = 64;
  constexpr int ROW_BLOCKS = N / ROWS_PER_BLOCK;
  constexpr int ROWS_PER_WORKER = 2;
  constexpr int WORKER_WARPS = ROWS_PER_BLOCK / ROWS_PER_WORKER;
  const int block = blockIdx.x;
  const int row_block = block % ROW_BLOCKS;
  const int bh = block / ROW_BLOCKS;
  const int head = bh % H;
  ...
  const int lane = threadIdx.x & 31;
  const bool is_producer = warp == 0;
  const int worker = warp - 1;
  const bool is_worker = worker >= 0 && worker < WORKER_WARPS;
  const int local_row0 = worker * ROWS_PER_WORKER;
  const int local_row1 = local_row0 + 1;
  const int row0 = row_block * ROWS_PER_BLOCK + local_row0;
  ...
  float st11 = 0.0f;
  if (is_worker) {
    st00 = state[state_base + row0 * N + col0];
    st01 = state[state_base + row0 * N + col1];
    st10 = state[state_base + row1 * N + col0];
    st11 = state[state_base + row1 * N + col1];
  }
  ...
  for (int t = 0; t < T; ++t) {
    ...
    if (is_worker) {
      const float dot0_part = st00 * kk_s[col0] + st01 * kk_s[col1];
      const float dot0 = rwkv7_warp_reduce_sum_broadcast(dot0_part);
      const float v0 = v_row_s[local_row0];
      const float new00 = st00 * w_s[col0] + v0 * k_s[col0] - dot0 * kk_s[col0] * a_s[col0];
      const float new01 = st01 * w_s[col1] + v0 * k_s[col1] - dot0 * kk_s[col1] * a_s[col1];
      st00 = new00;
      st01 = new01;
      const float rec0 = rwkv7_warp_reduce_sum_broadcast(new00 * r_s[col0] + new01 * r_s[col1]);
      if (lane == 0) {
        out[vec_base + row0] = static_cast<scalar_t>(rec0);
      }

      const float dot1_part = st10 * kk_s[col0] + st11 * kk_s[col1];
      const float dot1 = rwkv7_warp_reduce_sum_broadcast(dot1_part);
      const float v1 = v_row_s[local_row1];
      const float new10 = st10 * w_s[col0] + v1 * k_s[col0] - dot1 * kk_s[col0] * a_s[col0];
      const float new11 = st11 * w_s[col1] + v1 * k_s[col1] - dot1 * kk_s[col1] * a_s[col1];
      st10 = new10;
      st11 = new11;
      const float rec1 = rwkv7_warp_reduce_sum_broadcast(new10 * r_s[col0] + new11 * r_s[col1]);
      if (lane == 0) {
        out[vec_base + row1] = static_cast<scalar_t>(rec1);
      }
    }
    __syncthreads();
  }
  ...
  if (is_worker) {
    final_state[state_base + row0 * N + col0] = st00;
    final_state[state_base + row0 * N + col1] = st01;
    final_state[state_base + row1 * N + col0] = st10;
    final_state[state_base + row1 * N + col1] = st11;
  }
}

template <typename scalar_t, int ROWS_PER_BLOCK>
__global__ void rwkv7_state_scan_prep_n64_rowblock_halfwarp_pair_kernel(
    const scalar_t* __restrict__ r,
    const scalar_t* __restrict__ w_raw,
    const scalar_t* __restrict__ k_raw,
    const scalar_t* __restrict__ v_raw,
  ...
  const int local_row = worker * ROWS_PER_WORKER + half;
  const int row = row_block * ROWS_PER_BLOCK + local_row;
  const int col0 = lane16;
  const int col1 = lane16 + 16;
  const int col2 = lane16 + 32;
  const int col3 = lane16 + 48;
  ...
  if (is_worker) {
    st0 = state[state_base + row * N + col0];
    st1 = state[state_base + row * N + col1];
    st2 = state[state_base + row * N + col2];
    st3 = state[state_base + row * N + col3];
  }

  for (int t = 0; t < T; ++t) {
    const int vec_base = ((batch * T + t) * H + head) * N;
    if (is_producer) {
      const int pcol0 = lane;
      const int pcol1 = lane + 32;
      const float kr0 = static_cast<float>(k_raw[vec_base + pcol0]);
      const float kr1 = static_cast<float>(k_raw[vec_base + pcol1]);
      const float av0 = static_cast<float>(a[vec_base + pcol0]);
      const float av1 = static_cast<float>(a[vec_base + pcol1]);
      const float kk_raw0 = kr0 * static_cast<float>(k_k[param_base + pcol0]);
      const float kk_raw1 = kr1 * static_cast<float>(k_k[param_base + pcol1]);
      const float norm2 = rwkv7_warp_reduce_sum_broadcast(kk_raw0 * kk_raw0 + kk_raw1 * kk_raw1);
      const float norm_inv = rsqrtf(fmaxf(norm2, 1.0e-20f));
      const float kv0 = kr0 * (1.0f + (av0 - 1.0f) * static_cast<float>(k_a[param_base + pcol0]));
      const float kv1 = kr1 * (1.0f + (av1 - 1.0f) * static_cast<float>(k_a[param_base + pcol1]));

      r_s[pcol0] = static_cast<float>(r[vec_base + pcol0]);
      r_s[pcol1] = static_cast<float>(r[vec_base + pcol1]);
      w_s[pcol0] = __expf(-0.606531f / (1.0f + __expf(-static_cast<float>(w_raw[vec_base + pcol0]))));
      w_s[pcol1] = __expf(-0.606531f / (1.0f + __expf(-static_cast<float>(w_raw[vec_base + pcol1]))));
      k_s[pcol0] = kv0;
      k_s[pcol1] = kv1;
      a_s[pcol0] = av0;
      a_s[pcol1] = av1;
      kk_s[pcol0] = kk_raw0 * norm_inv;
      kk_s[pcol1] = kk_raw1 * norm_inv;

      if (row_block == 0) {
        k_out[vec_base + pcol0] = static_cast<scalar_t>(kv0);
        k_out[vec_base + pcol1] = static_cast<scalar_t>(kv1);
      }
      if (lane < ROWS_PER_BLOCK) {
        const int v_row = row_block * ROWS_PER_BLOCK + lane;
        const float vr = static_cast<float>(v_raw[vec_base + v_row]);
    ...
    if (is_worker) {
      ...
      const float state_dot_partial = st0 * kk0 + st1 * kk1 + st2 * kk2 + st3 * kk3;
      const float state_dot_kk = rwkv7_half_warp_reduce_sum_broadcast(state_dot_partial);
      const float v_row = v_row_s[local_row];
...
__global__ void rwkv7_state_scan_prep_n64_rowblock_precomputed_wk_half_warp_kernel(
  ...
  const int warp = threadIdx.x >> 5;
  const int lane = threadIdx.x & 31;
  const int row = row_block * ROWS_PER_BLOCK + warp;
  const int col0 = lane;
  const int col1 = lane + 32;
  ...
  for (int t = 0; t < T; ++t) {
    ...
    const float rv0 = static_cast<float>(r[vec_base + col0]);
    const float rv1 = static_cast<float>(r[vec_base + col1]);
    const float wv0 = static_cast<float>(w_prep[vec_base + col0]);
...
std::vector<torch::Tensor> rwkv7_state_scan_prep_forward_cuda(
  ...
  TORCH_CHECK(schedule_mode == 0 || schedule_mode == 1 || schedule_mode == 2 || schedule_mode == 3 || schedule_mode == 4 || schedule_mode == 5 || schedule_mode == 6 || schedule_mode == 7 || schedule_mode == 8 || schedule_mode == 9,
              "CUDA state-scan schedule_mode must be 0 (default), 1 (warp_specialized), 2 (warp2), 3 (head_reg16), 4 (warp_pipelined), 5 (warp_pair), 6 (precomputed_warp), 7 (warp_pipelined_half), 8 (head_reg8), or 9 (halfwarp_pair)");
  ...
  TORCH_CHECK(!(schedule_mode == 9 && rows_per_block == 1),
              "CUDA halfwarp_pair schedule requires rows_per_block=2, 4, 8, or 16");
  TORCH_CHECK(!(schedule_mode == 6 && precompute_mode != 3),
              "CUDA precomputed_warp schedule requires precompute_mode=wk_half");
  ...
  if (precompute_mode == 1) {
    ...
    rwkv7_state_scan_prep_n64_rowblock_warp_specialized_kernel<at::Half, 16, false, false><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
        ...
        v_out_ptr,
        B, T, H, has_v_gate);
  } else if (!write_kv && lanes_per_row == 64 && schedule_mode == 1 && rows_per_block == 8) {
    const dim3 row_grid(B * H * 8);
    const dim3 block(32 * (8 + 1));
    rwkv7_state_scan_prep_n64_rowblock_warp_specialized_kernel<at::Half, 8, false, false><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
        ...
        v_out_ptr,
        B, T, H, has_v_gate);
  } else if (!write_kv && lanes_per_row == 64 && schedule_mode == 1 && rows_per_block == 4) {
    const dim3 row_grid(B * H * 16);
    const dim3 block(32 * (4 + 1));
    rwkv7_state_scan_prep_n64_rowblock_warp_specialized_kernel<at::Half, 4, false, false><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
        ...
        v_out_ptr,
        B, T, H, has_v_gate);
  } else if (!write_kv && lanes_per_row == 64 && schedule_mode == 1 && rows_per_block == 2) {
    const dim3 row_grid(B * H * 32);
    const dim3 block(32 * (2 + 1));
    rwkv7_state_scan_prep_n64_rowblock_warp_specialized_kernel<at::Half, 2, false, false><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
        ...
        v_out_ptr,
        B, T, H, has_v_gate);
  } else if (!write_kv && lanes_per_row == 64 && schedule_mode == 1) {
    const dim3 row_grid(B * H * 64);
    const dim3 block(32 * (1 + 1));
    rwkv7_state_scan_prep_n64_rowblock_warp_specialized_kernel<at::Half, 1, false, false><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
    ...
    rwkv7_state_scan_prep_n64_head_reg8_kernel<at::Half><<<head_grid, block, 0, stream>>>(
        ...
        v_out_ptr,
        B, T, H, has_v_gate);
  } else if (lanes_per_row == 64 && schedule_mode == 7 && rows_per_block == 16) {
    const dim3 row_grid(B * H * 4);
    const dim3 block(32 * (16 + 1));
    rwkv7_state_scan_prep_n64_rowblock_warp_pipelined_half_shared_kernel<at::Half, 16><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
        ...
        v_out.data_ptr<at::Half>(),
        B, T, H, has_v_gate);
  } else if (lanes_per_row == 64 && schedule_mode == 7 && rows_per_block == 8) {
    const dim3 row_grid(B * H * 8);
    const dim3 block(32 * (8 + 1));
    rwkv7_state_scan_prep_n64_rowblock_warp_pipelined_half_shared_kernel<at::Half, 8><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
        ...
        v_out.data_ptr<at::Half>(),
        B, T, H, has_v_gate);
  } else if (lanes_per_row == 64 && schedule_mode == 7 && rows_per_block == 4) {
    const dim3 row_grid(B * H * 16);
    const dim3 block(32 * (4 + 1));
    rwkv7_state_scan_prep_n64_rowblock_warp_pipelined_half_shared_kernel<at::Half, 4><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
        ...
        v_out.data_ptr<at::Half>(),
        B, T, H, has_v_gate);
  } else if (lanes_per_row == 64 && schedule_mode == 7 && rows_per_block == 2) {
    const dim3 row_grid(B * H * 32);
    const dim3 block(32 * (2 + 1));
    rwkv7_state_scan_prep_n64_rowblock_warp_pipelined_half_shared_kernel<at::Half, 2><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
        ...
        v_out.data_ptr<at::Half>(),
        B, T, H, has_v_gate);
  } else if (lanes_per_row == 64 && schedule_mode == 7 && true) {
    const dim3 row_grid(B * H * 64);
    const dim3 block(32 * (1 + 1));
    rwkv7_state_scan_prep_n64_rowblock_warp_pipelined_half_shared_kernel<at::Half, 1><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
    ...
    rwkv7_state_scan_prep_n64_rowblock_warp2_kernel<at::Half, 1><<<row_grid, block, 0, stream>>>(
        ...
        v_out.data_ptr<at::Half>(),
        B, T, H, has_v_gate);
  } else if (lanes_per_row == 64 && schedule_mode == 5 && rows_per_block == 16) {
    const dim3 row_grid(B * H * 4);
    const dim3 block(32 * (1 + 16 / 2));
    rwkv7_state_scan_prep_n64_rowblock_warp_pair_kernel<at::Half, 16><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
        ...
        v_out.data_ptr<at::Half>(),
        B, T, H, has_v_gate);
  } else if (lanes_per_row == 64 && schedule_mode == 5 && rows_per_block == 8) {
    const dim3 row_grid(B * H * 8);
    const dim3 block(32 * (1 + 8 / 2));
    rwkv7_state_scan_prep_n64_rowblock_warp_pair_kernel<at::Half, 8><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
        ...
        v_out.data_ptr<at::Half>(),
        B, T, H, has_v_gate);
  } else if (lanes_per_row == 64 && schedule_mode == 5 && rows_per_block == 4) {
    const dim3 row_grid(B * H * 16);
    const dim3 block(32 * (1 + 4 / 2));
    rwkv7_state_scan_prep_n64_rowblock_warp_pair_kernel<at::Half, 4><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
        ...
        v_out.data_ptr<at::Half>(),
        B, T, H, has_v_gate);
  } else if (lanes_per_row == 64 && schedule_mode == 5 && rows_per_block == 2) {
    const dim3 row_grid(B * H * 32);
    const dim3 block(32 * (1 + 2 / 2));
    rwkv7_state_scan_prep_n64_rowblock_warp_pair_kernel<at::Half, 2><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
        ...
        v_out.data_ptr<at::Half>(),
        B, T, H, has_v_gate);
  } else if (lanes_per_row == 64 && schedule_mode == 9 && rows_per_block == 16) {
    const dim3 row_grid(B * H * 4);
    const dim3 block(32 * (1 + 16 / 2));
    rwkv7_state_scan_prep_n64_rowblock_halfwarp_pair_kernel<at::Half, 16><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
        ...
        v_out.data_ptr<at::Half>(),
        B, T, H, has_v_gate);
  } else if (lanes_per_row == 64 && schedule_mode == 9 && rows_per_block == 8) {
    const dim3 row_grid(B * H * 8);
    const dim3 block(32 * (1 + 8 / 2));
    rwkv7_state_scan_prep_n64_rowblock_halfwarp_pair_kernel<at::Half, 8><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
        ...
        v_out.data_ptr<at::Half>(),
        B, T, H, has_v_gate);
  } else if (lanes_per_row == 64 && schedule_mode == 9 && rows_per_block == 4) {
    const dim3 row_grid(B * H * 16);
    const dim3 block(32 * (1 + 4 / 2));
    rwkv7_state_scan_prep_n64_rowblock_halfwarp_pair_kernel<at::Half, 4><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
        ...
        v_out.data_ptr<at::Half>(),
        B, T, H, has_v_gate);
  } else if (lanes_per_row == 64 && schedule_mode == 9 && rows_per_block == 2) {
    const dim3 row_grid(B * H * 32);
    const dim3 block(32 * (1 + 2 / 2));
    rwkv7_state_scan_prep_n64_rowblock_halfwarp_pair_kernel<at::Half, 2><<<row_grid, block, 0, stream>>>(
        r.data_ptr<at::Half>(),
        w_raw.data_ptr<at::Half>(),
        k_raw.data_ptr<at::Half>(),
        v_raw.data_ptr<at::Half>(),
...
def cuda_state_scan_prep(
    ...
    *,
    ...
    When ``write_kv=False`` this is an opt-in raw-output probe: the CUDA scan
    keeps adjusted K/V local to the recurrence and returns empty K/V tensors, so
    the caller must pair it with a raw-K/V output-prep path.
    """
    ...
    if int(rows_per_block) != 1 and int(lanes_per_row) != 64:
        raise ValueError("cuda_state_scan_prep rows_per_block>1 requires lanes_per_row=64")
    if schedule is None:
        import os
    ...
    if schedule_name in {"", "0", "default", "normal", "rowblock", "none"}:
        ...
        raise ValueError(
            "cuda_state_scan_prep schedule must be default, warp_specialized, warp2, head_reg16, warp_pipelined, warp_pair, precomputed_warp, warp_pipelined_half, head_reg8, or halfwarp_pair"
        )
    if int(rows_per_block) != 1 and precompute_mode_id != 0 and not (schedule_id == 6 and precompute_mode_id == 3):
        raise ValueError(
            "cuda_state_scan_prep rows_per_block>1 requires precompute_mode=none, "
            "except precomputed_warp accepts wk_half"
        )
    if schedule_id and (int(lanes_per_row) != 64 or (precompute_mode_id != 0 and not (schedule_id == 6 and precompute_mode_id == 3))):
        raise ValueError(
            "cuda_state_scan_prep specialized schedules require lanes_per_row=64 and precompute_mode=none, "
            "except precomputed_warp requires wk_half"
        )
    ...
    if schedule_id == 9 and int(rows_per_block) == 1:
        raise ValueError("cuda_state_scan_prep halfwarp_pair uses rows_per_block=2, 4, 8, or 16")
    if schedule_id == 6 and precompute_mode_id != 3:
        raise ValueError("cuda_state_scan_prep precomputed_warp requires precompute_mode=wk_half")
    ...
    if inplace_kv is None:
        import os

        inplace_kv = os.environ.get("RWKV7_NATIVE_PREFILL_CUDA_STATE_SCAN_INPLACE_KV", "0").lower() not in {
            "0",
            "false",
            "no",
            "off",
    ...
    if bool(inplace_kv):
        if not bool(write_kv) or precompute_mode_id == 0 or bool(w_precomputed):
            raise ValueError(
