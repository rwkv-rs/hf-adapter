...
- `bitsandbytes` quantization is not an Apple path. Native MM8/MM4 now has
  ...
  Initial 4-session quant+Metal session-batch pressure rows now pass for 0.4B
  W8/W4 repeat=2 and 1.5B W8/W4 repeat=1. Current long-context ratio
  evidence still shows memory wins but speed gaps: 0.4B W8/W4 decode≈0.79x/0.81x
...
3. Extend the initial MLX packed W8/W4 affine and Metal quant paths beyond the current 0.4B/1.5B prompt512/1024 decode16 ratio rows and initial 4-session quant+Metal session-batch rows to longer repeat/session pressure, memory-pressure notes, and stable fp16-beating gates.
