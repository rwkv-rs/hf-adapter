...
- The MLX path is now a correctness-first recurrent reference backend, not a
  ...
  decode equality vs one-shot, interleaved multi-session equality vs one-shot with repeat-pressure summary rows, prompt/decode sweeps through 64-token prompts plus repeat pressure rows,
...
4. Extend the MLX recurrent reference and `MLXGenerationSession` beyond the current
   prompt64/decode4-or-2 sweep, prompt32/decode2 repeat row, and 3-session batch repeat row to longer prompt/decode matrices,
