...
- `bitsandbytes` quantization is not an Apple path. Native MM8/MM4 now has
  ...
  Current long-context ratio evidence shows memory wins but speed gaps: 0.4B
  W8/W4 decode≈0.79x/0.81x fp16 and 1.5B W8/W4 decode≈0.75x/0.84x fp16.
  Production Apple W8/W4 still needs repeat/session pressure, more M-series
  coverage, and stable speed rows that beat fp16 end to end.
...
3. Extend the initial MLX packed W8/W4 affine and Metal quant paths beyond the current 0.4B/1.5B prompt512/1024 decode16 ratio rows to repeat/session pressure rows, memory-pressure notes, and stable fp16-beating gates.
