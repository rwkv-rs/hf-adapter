...
- `bitsandbytes` quantization is not an Apple path. Native MM8/MM4 now has
  ...
  backend-count telemetry and routes W4 small batches to Metal while keeping W8
  on affine by default; 0.4B W4/W8 auto strict compare rows both pass. Current
  long-context ratio evidence shows
  memory wins and one 0.4B W4 prompt2048/decode128 fp16-beating row
  (decode≈1.04x fp16), but W8 and 1.5B W4 remain below fp16 in the same row
  (1.5B W8/W4 decode≈0.68x/0.73x). Production Apple W8/W4
