...
| MacBook Air / Apple M5 | 16GB | 26.5 | MLX 0.31.2 | MLX GPU | MLX quant projection microbench | PASS (`scripts/mlx_quant_projection_bench.py --rows 1,4 --bits 4,8 --in-features 2048 --out-features 2048`; W4 footprint≈0.252x, W8≈0.502x. Isolated projection results show W4/Metal rows=4 can beat dense fp16 for this microbench (`1.11x`, auto `1.38x`), while W4 rows=1 remains below dense (`0.39x-0.44x`). W8/Metal improves over W8 affine but remains below dense (`0.87x` rows=1, `0.67x` rows=4), and W8 auto still routes to affine by default because the session-batch exactness guard remains open. This identifies projection launch/reduction fusion as the next speed target.) |
