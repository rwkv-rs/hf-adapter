...
| MacBook Air / Apple M5 | 16GB | 26.5 | MLX 0.31.2 | MLX GPU | MLX/Metal packed W8/W4 fused dequant-projection seam | PASS (`--quant-backend metal --wkv-backend metal`; `rwkv7-g1d-0.1b-hf` lm_head-only prompt32/decode2: W8 footprint≈0.502635, peak≈338MB, prefill≈88.01 tok/s, decode≈110.59 tok/s; W4 footprint≈0.252635, peak≈313MB, prefill≈105.80 tok/s, decode≈75.27 tok/s. `rwkv7-g1d-0.4b-hf` 49 FFN/lm_head modules prompt16/decode1: W8 footprint≈0.502327, peak≈642MB, prefill≈49.48 tok/s, decode≈49.65 tok/s; W4 footprint≈0.252327, peak≈508MB, prefill≈47.99 tok/s, decode≈49.38 tok/s. `rwkv7-g1g-1.5b-hf` 49 FFN/lm_head modules prompt16/decode1: W8 footprint≈0.501190, peak≈2134MB, prefill≈19.47 tok/s, decode≈21.59 tok/s; W4 footprint≈0.251190, peak≈1664MB, prefill≈18.63 tok/s, decode≈24.04 tok/s. Pressure matrix rows now cover 0.4B/1.5B prompt128/256 decode4/8 chunk64, prompt512/1024 decode16 chunk256, and prompt2048/decode128 chunk512: 0.4B prompt2048 W8 peak≈649MB prefill/decode≈43.54/42.03 tok/s, 0.4B prompt2048 W4 peak≈515MB prefill/decode≈50.18/49.84 tok/s, 1.5B prompt2048 W8 peak≈2147MB prefill/decode≈21.03/18.59 tok/s, and 1.5B prompt2048 W4 peak≈1677MB prefill/decode≈20.97/19.74 tok/s. The optimized W4 auto route records `metal=202885` and improves prompt2048/decode128 to 0.4B peak≈515MB prefill/decode≈60.61/59.73 tok/s and 1.5B peak≈1677MB prefill/decode≈27.64/20.42 tok/s. New prompt4096/decode256 chunk1024 rows keep chunked/full max_abs=0.0 and record 0.4B fp16 vs W4 auto prefill/decode≈94.08/75.38 vs 62.01/55.29 tok/s, and 1.5B fp16 vs W4 auto≈35.34/33.21 vs 27.40/25.46 tok/s; W4 peak remains≈0.56x/0.54x fp16 but speed is still below fp16 at this longer shape. Repeat/session pressure and stable fp16-beating gates remain open.) |
| MacBook Air / Apple M5 | 16GB | 26.5 | MLX 0.31.2 | MLX GPU | MLX/Metal W8/W4 vs fp16 ratio gate | GAP (`--quant-backend metal --wkv-backend metal` compared with same-shape `--quantization none --wkv-backend metal`. prompt512/1024 decode16: 0.4B fp16 peak≈929MB, decode≈52.37 tok/s, W8/W4 peak ratios≈0.71x/0.57x and decode ratios≈0.79x/0.81x; 1.5B fp16 peak≈3110MB, decode≈23.15 tok/s, W8/W4 peak ratios≈0.70x/0.55x and decode ratios≈0.75x/0.84x. New prompt2048/decode128: 0.4B fp16 peak≈916MB, decode≈47.97 tok/s, W8/W4 peak ratios≈0.71x/0.56x and decode ratios≈0.88x/1.04x; 1.5B fp16 peak≈3084MB, decode≈27.20 tok/s, W8/W4 peak ratios≈0.70x/0.54x and decode ratios≈0.68x/0.73x. The optimized W4 auto route improves prompt2048/decode128 to 0.4B W4 decode≈1.25x fp16 with peak≈0.56x and 1.5B W4 decode≈0.75x fp16 with peak≈0.54x. At prompt4096/decode256 W4 auto remains below fp16 (0.4B decode≈0.73x, 1.5B decode≈0.77x) while keeping peak≈0.56x/0.54x. Memory target is moving in the right direction and 0.4B W4 has stronger fp16-beating medium-long decode evidence, but stable W8/W4 speed > fp16 across sizes/modes is still open.) |
...
Memory-pressure note after the latest 1.5B MLX prompt4096/decode256 and 12-step
...
| M-series 16GB+ | 16GB+ | 1.5B HF | fp16 inference / fp32 LoRA smoke | mps | load/generate + MPS prompt sweep through 512 tokens / decode 8 + MLX prompt4096/decode256 + PEFT manual + Trainer/SFT/DPO/GRPO 1/2/3/5/10/12-step + peak memory + finite trainable update |
...
- `bitsandbytes` quantization is not an Apple path. Native MM8/MM4 now has
  ...
  Quant+Metal session-batch pressure rows now pass for 0.4B W8/W4 4-session
  repeat=2 and 6-session repeat=3, plus 1.5B W8/W4 4-session repeat=1 and
  5-session repeat=2. The opt-in equal-round batched session backend now has
  ...
  longer 0.4B/1.5B rounds8,8 repeat pressure; default W8/Metal auto stays
  ...
  while prompt4096/decode256 shows W4 below fp16 despite peak≈0.56x/0.54x,
  1.5B W4 auto improves to decode≈0.75x fp16, and W8 remains below fp16
  (1.5B W8 decode≈0.68x). Production Apple W8/W4
...
- Long-running full-size training on MPS is not claimed yet. Tiny native Trainer
  ...
  smoke now pass. Longer MLX 1.5B decode through 256 tokens now passes; longer production-style training/decode and larger Apple machines
...
2. Extend 1.5B beyond 12-step Trainer/TRL and prompt4096/decode256 MLX sweep to
   longer production-style training/decode and memory-pressure notes.
3. Extend the initial MLX packed W8/W4 affine and Metal quant paths beyond the current 0.4B/1.5B prompt4096/decode256 ratio rows, 0.4B 8-session repeat=2, and 1.5B 5-session repeat=2 quant+Metal session-batch rows to still-longer repeat/session pressure, memory-pressure notes, and stable fp16-beating gates.
4. Extend the MLX recurrent reference and `MLXGenerationSession` / batched session backend beyond the current
   0.1B prompt256/decode8, 0.4B/1.5B prompt4096/decode256 matrices, 0.4B 8-session repeat=2, and 1.5B 5-session repeat=2 rows to longer prompt distributions,
