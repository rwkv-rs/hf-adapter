...
| MacBook Air / Apple M5 | 16GB | 26.5 | MLX 0.31.2 | MLX GPU | MLX/Metal packed W8/W4 fused dequant-projection seam | PASS (`--quant-backend metal --wkv-backend metal`; `rwkv7-g1d-0.1b-hf` lm_head-only prompt32/decode2: W8 footprint≈0.502635, peak≈338MB, prefill≈88.01 tok/s, decode≈110.59 tok/s; W4 footprint≈0.252635, peak≈313MB, prefill≈105.80 tok/s, decode≈75.27 tok/s. `rwkv7-g1d-0.4b-hf` 49 FFN/lm_head modules prompt16/decode1: W8 footprint≈0.502327, peak≈642MB, prefill≈49.48 tok/s, decode≈49.65 tok/s; W4 footprint≈0.252327, peak≈508MB, prefill≈47.99 tok/s, decode≈49.38 tok/s. `rwkv7-g1g-1.5b-hf` 49 FFN/lm_head modules prompt16/decode1: W8 footprint≈0.501190, peak≈2134MB, prefill≈19.47 tok/s, decode≈21.59 tok/s; W4 footprint≈0.251190, peak≈1664MB, prefill≈18.63 tok/s, decode≈24.04 tok/s. Pressure matrix rows now cover 0.4B/1.5B prompt128/256 decode4/8 chunk64 and prompt512/1024 decode16 chunk256: 0.4B long-context W8 peak≈662MB min prefill/decode≈45.86/41.19 tok/s, 0.4B long-context W4 peak≈528MB min prefill/decode≈44.66/42.58 tok/s, 1.5B long-context W8 peak≈2172MB min prefill/decode≈19.25/17.33 tok/s, and 1.5B long-context W4 peak≈1703MB min prefill/decode≈19.60/19.33 tok/s. Repeat/session pressure and stable fp16-beating gates remain open.) |
| MacBook Air / Apple M5 | 16GB | 26.5 | MLX 0.31.2 | MLX GPU | MLX/Metal W8/W4 vs fp16 ratio gate | GAP (`--quant-backend metal --wkv-backend metal` compared with same prompt512/1024 decode16 `--quantization none --wkv-backend metal`; 0.4B fp16 peak≈929MB, prefill≈58.51 tok/s, decode≈52.37 tok/s; 0.4B W8/W4 peak ratios≈0.71x/0.57x and decode ratios≈0.79x/0.81x. 1.5B fp16 peak≈3110MB, prefill≈26.67 tok/s, decode≈23.15 tok/s; 1.5B W8/W4 peak ratios≈0.70x/0.55x and decode ratios≈0.75x/0.84x. Memory target is moving in the right direction; stable W8/W4 speed > fp16 is still open.) |
...
- `bitsandbytes` quantization is not an Apple path. Native MM8/MM4 now has
  ...
  first W4 rows on 0.4B 6-session repeat=2 and 1.5B 5-session repeat=1 with
  one-shot equality and aggregate round tok/s telemetry. Strict W8/Metal batched
  longer decode still has a batch-exactness gap, so `SESSION_BACKEND=auto` now
  records `auto_mm8_metal_batch_exactness_guard` and falls back to sequential
  for safe W8 rows until the fused batch path is fixed. Backend-compare rows
  ...
  token index 6 for the short prompt. Current long-context ratio evidence still shows memory
  wins but speed gaps: 0.4B W8/W4 decode≈0.79x/0.81x
  fp16 and 1.5B W8/W4 decode≈0.75x/0.84x fp16. Production Apple W8/W4
...
3. Extend the initial MLX packed W8/W4 affine and Metal quant paths beyond the current 0.4B/1.5B prompt512/1024 decode16 ratio rows, 0.4B 6-session repeat=3, and 1.5B 5-session repeat=2 quant+Metal session-batch rows to still-longer repeat/session pressure, memory-pressure notes, and stable fp16-beating gates.
