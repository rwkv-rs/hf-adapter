...
- `bitsandbytes` quantization is not an Apple path. Native MM8/MM4 now has
  ...
  so this is now localized for the next W8 exactness fix. `--quant-backend auto` now records
  backend-count telemetry and routes W4 normal prefill/decode rows to Metal while
  keeping W8 on affine by default; 0.4B W4/W8 auto strict compare rows both pass.
  ...
  while 1.5B W4 auto improves to decode≈0.75x fp16 and W8 remains below fp16
