...
| MLX DPLR/WY model prefill | opt-in real-model pass | `MLXRWKV7Model` now has layer-major sequence prefill and partial-chunk padding. M5 prompt512 medians improve recurrent→DPLR by≈1.56x/1.49x/3.29x for 0.1B/0.4B/1.5B fp16 and≈1.47x for 0.4B W4; 8-token continuations match and logits/state max-abs≤0.0625/0.128. Peak rises≈8%-24%, so default remains recurrent and `auto` requires≥128 tokens. |
...
| MLX recurrent backend / Metal backend | initial MLX recurrent + Metal WKV + Metal packed quant + DPLR seams | Optional `.[mlx]` install, `rwkv7_hf.mlx_bridge`, `rwkv7_hf.mlx_model`, `rwkv7_hf.mlx_quant`, `rwkv7_hf.mlx_wkv`, `rwkv7_hf.mlx_dplr_prefill`, and the Apple scripts now validate HF safetensor → MLX array/export, recurrent/cache/session behavior, prompt/decode sweeps, packed W8/W4 projections, custom Metal WKV/quant paths, parity-gated lazy prefill evaluation batching, and synthetic compact-WY three-stage kernels. Production fused Metal still needs tiled summary reduction, partial chunks, layer-major model integration, longer pressure, and cross-device tuning. |
...
7. Continue the now-working MLX/Metal DPLR/WY scaffold: tile/parallelize the
   summary kernel, reduce its peak/cache effect, extend the new layer-major
   real-model route to long prompts and broader quality gates, and then fuse
   WKV and W8/W4 projections until full-model prefill/quant gates pass.
