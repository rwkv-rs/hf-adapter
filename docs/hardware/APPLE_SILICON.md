...
- `bitsandbytes` quantization is not an Apple path. Native MM8/MM4 now has
  ...
  `SESSION_BACKEND=batched_stable` row closes this 0.4B W8 compare gate on
  6 sessions while default W8 auto stays guarded pending longer coverage.
  `--quant-backend auto` now records
