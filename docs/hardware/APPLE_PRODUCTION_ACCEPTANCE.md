...
Manifest 版本：`2026-07-13.1`。当前状态由已提交 JSONL 和明确登记的文件证据实时计算，不从源码功能名或说明文字推断 PASS。
...
| ⬜ MISSING | `train.mps.state_tuning` | RWKV State Tuning | 提供可训练初始 RWKV recurrent/shift state adapter：base 权重冻结，按 batch 安全展开且无别名；0.1B/0.4B/1.5B 在 MPS 上通过有限 loss、非零梯度与更新、Trainer 训练、保存重载、断点续训和 generate continuation，重载后 logits/state/greedy 在声明容差内一致。 | no machine-verifiable proof registered |
