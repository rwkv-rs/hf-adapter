...
English version: [`INFERENCE_WORKFLOWS.md`](INFERENCE_WORKFLOWS.md)
...
[`TRAINING_WORKFLOWS_ZH.md`](TRAINING_WORKFLOWS_ZH.md)。
...
让 AI 先读本页，并且一次只执行一个编号章节。它必须先检查真实设备，替换为
已经存在且检查通过的 `MODEL`，执行前复述最终命令，遇到非零退出码立即停止
修复，最后报告文档规定的通过标记。禁止把 cache primitive 写成完整生产服务，
也禁止把 smoke 写成性能结论。
