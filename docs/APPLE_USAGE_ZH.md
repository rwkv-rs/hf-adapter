...
English version: [`APPLE_USAGE.md`](APPLE_USAGE.md)
...
先告诉 AI 要走 MPS、MLX 还是 CoreML。它必须确认 Apple Silicon、可用内存、
模型大小和现有路径；转换前请求确认；一次只跑一个章节；报告退出码、精确设备/
runtime、生成/对齐证据和内存。禁止把 dry-run 计划写成部署完成，也不能把 M5
证据外推到另一台 Mac。
