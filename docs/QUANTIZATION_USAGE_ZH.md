...
证据边界见 [`APPLE_USAGE_ZH.md`](APPLE_USAGE_ZH.md#4-packed-mlx-w8w4)。

## 7. 交给 AI 执行时

AI 必须先跑 dense baseline，再只跑一个量化模式，并报告模块数、footprint、峰值
显存、logits/greedy 门槛、精确 GPU 和 shape。只有配对端到端时间通过时才能写
“更快”；“加载成功”“功能通过”“更省内存”是三个不同结论。
