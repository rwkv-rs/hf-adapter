...
对齐，Native/no-FLA B16 用于官方 shell 形状的逐 tensor、三 seed、断点恢复和显存
稳定性验收。其他 Ampere+ 设备可以按同一接口试跑，但必须通过自己的精确卡验收后
才能继承结论。
...
官方 shell 形状的 Native B16 证据、完整三 seed 图、500+500 恢复哈希和稳态显存
采样见
...
曲线原子写入同一个 checkpoint。以下命令展示 500+500 恢复；`SEQ` 必须包含 1,000
步，`INIT_SHA` 是转换前官方初始化的 SHA256：

```bash
export SEQ=/path/to/sequence-1000.safetensors
...
python bench/bench_train_temp_alignment.py converge-hf "${COMMON[@]}" \
  ...
  --checkpoint-every 250 --stop-after-step 500

python bench/bench_train_temp_alignment.py converge-hf "${COMMON[@]}" \
  --output-json resumed.json --resume-from "$CKPT" \
  --checkpoint-out "$CKPT" --checkpoint-every 250
```

第一条命令的可观察通过标准是 `status=partial`、`failure=null`、
`steps_completed=500`；第二条必须为 `status=pass`、`steps_completed=1000`，且
...
一致；官方/Native 三 seed 都是 `3/3` complete/deep-success，500+500 恢复通过，
第 50 到 1,000 步 reserved 显存零增长。Native 中位训练吞吐为官方 `0.9499x`，
因此当前结论是数值与有界稳定性通过，训练速度尚未持平。
...
当前不把该后端描述为 FP16/FP32、padding、可变长度、多卡或所有 GPU 的生产实现。
