...
def analyze(rows: list[dict[str, Any]], args: argparse.Namespace) -> dict[str, Any]:
    ...
    native_prefill_scan = latest_by_key(
        ...
        lambda r: (
            ...
            bool(r.get("prefill_cuda_state_scan_sk_effective")),
            ...
            bool(r.get("prefill_fused_norm_mix_effective")),
            bool(r.get("prefill_fused_state_prep_effective")),
            bool(r.get("prefill_fused_state_scan_effective")),
            bool(r.get("prefill_fused_state_scan_correction_effective")),
            bool(r.get("prefill_fused_state_scan_raw_output_effective")),
            bool(r.get("prefill_fused_state_scan_sk_output_effective")),
            r.get("prefill_state_prep_w_dtype"),
            bool(r.get("prefill_fused_output_effective")),
            bool(r.get("prefill_tail_norm_slice_effective")),
            bool(r.get("prefill_fused_wavg_lora_effective")),
            bool(r.get("prefill_fused_shift_wavg_lora_effective")),
            r.get("prefill_fused_shift_wavg_lora_block_m"),
            r.get("prefill_fused_shift_wavg_lora_block_r"),
            r.get("prefill_fused_shift_wavg_lora_block_k"),
            r.get("prefill_fused_shift_wavg_lora_down_warps"),
            r.get("prefill_fused_shift_wavg_lora_up_warps"),
            bool(r.get("prefill_fused_shift_wavg_lora_a_sigmoid_effective")),
            bool(r.get("prefill_fused_shift_wavg_lora_prev_cache_effective")),
            bool(r.get("prefill_fused_projection_effective")),
    ...
    state_scan_output_micro = latest_by_key(
        [r for r in rows if r.get("axis") == "state_scan_output_micro" and r.get("backend") == "hf_adapter"],
        lambda r: (
            r.get("bench_case"),
            r.get("batch_size"),
            r.get("seq_len"),
    ...
    dplr_compact_wy_stage_proto = latest_by_key(
        [r for r in rows if r.get("axis") == "dplr_compact_wy_stage_proto" and r.get("backend") == "hf_adapter"],
        lambda r: (
            r.get("B", r.get("batch_size")),
            r.get("T", r.get("tokens")),
            r.get("H", r.get("heads")),
            ...
            albatross_prefill_comparison.append(
                {
                    ...
                    "hf_scan_num_stages": hf.get("scan_num_stages"),
                    "hf_scan_algebraic_output": hf.get("scan_algebraic_output"),
                    "hf_scan_nomask64": hf.get("scan_nomask64"),
                    "hf_prefill_cuda_state_scan_effective": hf.get("prefill_cuda_state_scan_effective"),
                    "hf_prefill_cuda_state_scan_precompute": hf.get("prefill_cuda_state_scan_precompute"),
                    "hf_prefill_cuda_state_scan_precompute_mode": hf.get("prefill_cuda_state_scan_precompute_mode"),
                    "hf_prefill_cuda_state_scan_rows_per_block": hf.get("prefill_cuda_state_scan_rows_per_block"),
                    "hf_prefill_cuda_state_scan_schedule": hf.get("prefill_cuda_state_scan_schedule"),
                    ...
                    "hf_prefill_fused_state_scan_raw_output_effective": hf.get("prefill_fused_state_scan_raw_output_effective"),
                    "hf_prefill_fused_state_scan_sk_output_effective": hf.get("prefill_fused_state_scan_sk_output_effective"),
        ...
        if speedups:
            ...
            for row in scan_output_rows:
                base_candidates = [
                    ...
                    and bool(r.get("prefill_cuda_state_scan_sk_effective")) == bool(row.get("prefill_cuda_state_scan_sk_effective"))
            ...
            algebraic_rows = [r for r in native_prefill_scan if r.get("scan_algebraic_output")]
            algebraic_ratios: list[float] = []
            for row in algebraic_rows:
                base_candidates = [
                    ...
                    and bool(r.get("prefill_fused_shift_mix_effective")) == bool(row.get("prefill_fused_shift_mix_effective"))
                    and bool(r.get("prefill_fused_state_prep_effective")) == bool(row.get("prefill_fused_state_prep_effective"))
                    and bool(r.get("prefill_fused_state_scan_effective")) == bool(row.get("prefill_fused_state_scan_effective"))
                    and bool(r.get("prefill_fused_output_effective")) == bool(row.get("prefill_fused_output_effective"))
                    and bool(r.get("prefill_fused_wavg_lora_effective")) == bool(row.get("prefill_fused_wavg_lora_effective"))
                    and not bool(r.get("scan_algebraic_output"))
                ]
                base = max(base_candidates, key=lambda r: float(r.get("native_prefill_tokps_total") or 0.0), default=None)
                row_tokps = num(row, "native_prefill_tokps_total")
                base_tokps = num(base, "native_prefill_tokps_total")
                if row_tokps is not None and base_tokps:
                    algebraic_ratios.append(float(row_tokps) / float(base_tokps))
            if algebraic_ratios:
                focus.append(
                    f"native prefill algebraic scan-output opt-in A/B ratio min={min(algebraic_ratios):.3f}x "
                    f"max={max(algebraic_ratios):.3f}x; keep disabled unless it repeatedly beats the normal full-head scan"
                )
            nomask_rows = [r for r in native_prefill_scan if r.get("scan_nomask64")]
            nomask_ratios: list[float] = []
            for row in nomask_rows:
                base_candidates = [
                    r
                    for r in native_prefill_scan
                    if r.get("batch_size") == row.get("batch_size")
                    ...
                    and bool(r.get("prefill_fused_output_effective")) == bool(row.get("prefill_fused_output_effective"))
                    and bool(r.get("prefill_fused_wavg_lora_effective")) == bool(row.get("prefill_fused_wavg_lora_effective"))
                    and not bool(r.get("scan_nomask64"))
                ]
                base = max(base_candidates, key=lambda r: float(r.get("native_prefill_tokps_total") or 0.0), default=None)
                row_tokps = num(row, "native_prefill_tokps_total")
                base_tokps = num(base, "native_prefill_tokps_total")
                if row_tokps is not None and base_tokps:
                    nomask_ratios.append(float(row_tokps) / float(base_tokps))
            if nomask_ratios:
                focus.append(
                    f"native prefill N=64 no-mask state-scan opt-in A/B ratio min={min(nomask_ratios):.3f}x "
                    f"max={max(nomask_ratios):.3f}x; promote only if it repeatedly beats the generic full-head scan"
                )
            cuda_scan_rows = [r for r in native_prefill_scan if r.get("prefill_cuda_state_scan_effective")]
            cuda_scan_ratios: list[float] = []
            for row in cuda_scan_rows:
                base_candidates = [
                    r
                    for r in native_prefill_scan
                    if r.get("batch_size") == row.get("batch_size")
                    ...
                    and bool(r.get("prefill_fused_output_effective")) == bool(row.get("prefill_fused_output_effective"))
                    and bool(r.get("prefill_fused_wavg_lora_effective")) == bool(row.get("prefill_fused_wavg_lora_effective"))
                    and not bool(r.get("prefill_cuda_state_scan_effective"))
                ]
                base = max(base_candidates, key=lambda r: float(r.get("native_prefill_tokps_total") or 0.0), default=None)
                row_tokps = num(row, "native_prefill_tokps_total")
                base_tokps = num(base, "native_prefill_tokps_total")
                if row_tokps is not None and base_tokps:
                    cuda_scan_ratios.append(float(row_tokps) / float(base_tokps))
            if cuda_scan_ratios:
                focus.append(
            ...
            if warp_rows:
                ...
                for key in sorted({(r.get("batch_size"), r.get("prompt_tokens")) for r in warp_rows}, key=str):
                    ...
                    best_warp_by_case.append(
                        f"bsz={key[0]} tok={key[1]} warps={best.get('scan_num_warps')} stages={best.get('scan_num_stages')} tokps={best.get('native_prefill_tokps_total')}"
                    )
                focus.append(
    ...
    state_scan_output_summary = next(
        (r for r in state_scan_output_micro if r.get("bench_case") == "summary"),
        None,
    )
    if state_scan_output_summary:
        focus.append(
            "state-scan/output micro-split row present: "
            f"components={state_scan_output_summary.get('component_ms')} "
            f"deltas={state_scan_output_summary.get('delta_ms')}; "
            "use to choose the next sk-scale output/scan boundary"
        )
    ...
    return {
        ...
        "native_prefill_scan": [
            compact(r, ["_lineno", "status", "dtype", "device", "code_source", "native_jit_module", "effective_model_path", "batch_size", "prompt_tokens", "tokens_total", "fused_scan_requested", "scan_block_m", "scan_num_warps", "scan_num_stages", "scan_algebraic_output", "scan_nomask64", "scan_precompute_w", "scan_precompute_w_dtype", "prefill_rkv_bmm_requested", "prefill_rkv_bmm_effective", "prefill_rkv_bmm_max_rows", "prefill_cuda_state_scan_requested", "prefill_cuda_state_scan_effective", "prefill_cuda_state_scan_sk_requested", "prefill_cuda_state_scan_sk_effective", "prefill_cuda_state_scan_lanes", "prefill_cuda_state_scan_precompute", "prefill_cuda_state_scan_precompute_mode", "prefill_cuda_state_scan_reuse_precompute", "prefill_cuda_state_scan_reuse_precompute_effective", "prefill_cuda_state_scan_inplace_kv", "prefill_cuda_state_scan_inplace_kv_effective", "prefill_cuda_state_scan_inplace_v", "prefill_cuda_state_scan_inplace_v_effective", "prefill_cuda_state_scan_inplace_kka", "prefill_cuda_state_scan_inplace_kka_effective", "prefill_cuda_state_scan_rows_per_block", "prefill_cuda_state_scan_schedule", "prefill_cuda_state_scan_w_precomputed", "prefill_fused_scan_output_requested", "prefill_fused_scan_output_effective", "prefill_fused_clampw_scan_requested", "prefill_fused_clampw_scan_effective", "prefill_dplr_scan_requested", "prefill_dplr_scan_effective", "prefill_dplr_algorithm", "prefill_dplr_chunk_size", "prefill_dplr_compact_block_n", "prefill_dplr_compact_block_r", "prefill_dplr_compact_prefix_block_m", "prefill_dplr_compact_start_state_dtype", "prefill_dplr_compact_output_only", "prefill_dplr_compact_recompute_starts", "prefill_dplr_compact_prefix_shared", "prefill_dplr_compact_prefix_shared_group_size", "prefill_fused_shift_mix_requested", "prefill_fused_shift_mix_effective", "prefill_fused_norm_mix_requested", "prefill_fused_norm_mix_effective", "prefill_fused_state_prep_requested", "prefill_fused_state_prep_effective", "prefill_fused_state_scan_requested", "prefill_fused_state_scan_effective", "prefill_fused_state_scan_correction_requested", "prefill_fused_state_scan_correction_effective", "prefill_fused_state_scan_raw_output_requested", "prefill_fused_state_scan_raw_output_effective", "prefill_fused_state_scan_sk_output_requested", "prefill_fused_state_scan_sk_output_effective", "prefill_fused_output_requested", "prefill_fused_output_effective", "prefill_tail_norm_slice_requested", "prefill_tail_norm_slice_effective", "prefill_fused_wavg_lora_requested", "prefill_fused_wavg_lora_effective", "prefill_fused_wavg_lora_max_m", "prefill_fused_shift_wavg_lora_requested", "prefill_fused_shift_wavg_lora_effective", "prefill_fused_shift_wavg_lora_block_m", "prefill_fused_shift_wavg_lora_block_r", "prefill_fused_shift_wavg_lora_block_k", "prefill_fused_shift_wavg_lora_down_warps", "prefill_fused_shift_wavg_lora_up_warps", "prefill_fused_shift_wavg_lora_lean_down", "prefill_fused_shift_wavg_lora_lean_up", "prefill_fused_shift_wavg_lora_g_mid_output_requested", "prefill_fused_shift_wavg_lora_g_mid_output_effective", "prefill_fused_shift_wavg_lora_w_decay_requested", "prefill_fused_shift_wavg_lora_w_decay_effective", "prefill_fused_shift_wavg_lora_a_sigmoid_requested", "prefill_fused_shift_wavg_lora_a_sigmoid_effective", "prefill_fused_shift_wavg_lora_prev_cache_requested", "prefill_fused_shift_wavg_lora_prev_cache_effective", "prefill_fused_projection_requested", "prefill_fused_projection_effective", "prefill_fused_projection_max_m", "prefill_fused_projection_block_m", "prefill_fused_projection_block_r", "prefill_fused_projection_block_k", "fast_token_backend_after_native_prefill", "hf_prefill_ms", "native_prefill_ms", "native_vs_hf_speedup", "hf_prefill_tokps_total", "native_prefill_tokps_total", "max_abs_diff", "min_cosine", "greedy_match", "decode_after_prefill_max_abs_diff", "decode_after_prefill_greedy_match", "peak_vram_mb"])
            for r in native_prefill_scan
        ],
        "native_prefill_breakdown": [
            compact(r, ["_lineno", "status", "dtype", "device", "model_size_label", "batch_size", "prompt_tokens", "tokens_total", "fused_scan_requested", "scan_block_m", "scan_num_warps", "scan_num_stages", "scan_algebraic_output", "scan_nomask64", "scan_precompute_w", "scan_precompute_w_dtype", "prefill_rkv_bmm_requested", "prefill_rkv_bmm_effective", "prefill_rkv_bmm_max_rows", "prefill_cuda_state_scan_requested", "prefill_cuda_state_scan_effective", "prefill_cuda_state_scan_sk_requested", "prefill_cuda_state_scan_sk_effective", "prefill_cuda_state_scan_lanes", "prefill_cuda_state_scan_precompute", "prefill_cuda_state_scan_precompute_mode", "prefill_cuda_state_scan_reuse_precompute", "prefill_cuda_state_scan_reuse_precompute_effective", "prefill_cuda_state_scan_inplace_kv", "prefill_cuda_state_scan_inplace_kv_effective", "prefill_cuda_state_scan_inplace_v", "prefill_cuda_state_scan_inplace_v_effective", "prefill_cuda_state_scan_inplace_kka", "prefill_cuda_state_scan_inplace_kka_effective", "prefill_cuda_state_scan_rows_per_block", "prefill_cuda_state_scan_schedule", "prefill_cuda_state_scan_w_precomputed", "fine_attention_breakdown", "layer_breakdown", "prefill_fused_scan_output_requested", "prefill_fused_scan_output_effective", "prefill_fused_clampw_scan_requested", "prefill_fused_clampw_scan_effective", "prefill_dplr_scan_requested", "prefill_dplr_scan_effective", "prefill_dplr_algorithm", "prefill_dplr_chunk_size", "prefill_dplr_compact_block_n", "prefill_dplr_compact_block_r", "prefill_dplr_compact_prefix_block_m", "prefill_dplr_compact_start_state_dtype", "prefill_dplr_compact_output_only", "prefill_dplr_compact_recompute_starts", "prefill_dplr_compact_prefix_shared", "prefill_dplr_compact_prefix_shared_group_size", "prefill_fused_shift_mix_requested", "prefill_fused_shift_mix_effective", "prefill_fused_norm_mix_requested", "prefill_fused_norm_mix_effective", "prefill_fused_state_prep_requested", "prefill_fused_state_prep_effective", "prefill_fused_state_scan_requested", "prefill_fused_state_scan_effective", "prefill_fused_state_scan_correction_requested", "prefill_fused_state_scan_correction_effective", "prefill_fused_state_scan_raw_output_requested", "prefill_fused_state_scan_raw_output_effective", "prefill_fused_state_scan_sk_output_requested", "prefill_fused_state_scan_sk_output_effective", "prefill_fused_output_requested", "prefill_fused_output_effective", "prefill_tail_norm_slice_requested", "prefill_tail_norm_slice_effective", "prefill_fused_wavg_lora_requested", "prefill_fused_wavg_lora_effective", "prefill_fused_wavg_lora_max_m", "prefill_fused_shift_wavg_lora_requested", "prefill_fused_shift_wavg_lora_effective", "prefill_fused_shift_wavg_lora_block_m", "prefill_fused_shift_wavg_lora_block_r", "prefill_fused_shift_wavg_lora_block_k", "prefill_fused_shift_wavg_lora_down_warps", "prefill_fused_shift_wavg_lora_up_warps", "prefill_fused_shift_wavg_lora_lean_down", "prefill_fused_shift_wavg_lora_lean_up", "prefill_fused_shift_wavg_lora_g_mid_output_requested", "prefill_fused_shift_wavg_lora_g_mid_output_effective", "prefill_fused_shift_wavg_lora_w_decay_requested", "prefill_fused_shift_wavg_lora_w_decay_effective", "prefill_fused_shift_wavg_lora_a_sigmoid_requested", "prefill_fused_shift_wavg_lora_a_sigmoid_effective", "prefill_fused_shift_wavg_lora_prev_cache_requested", "prefill_fused_shift_wavg_lora_prev_cache_effective", "prefill_fused_projection_requested", "prefill_fused_projection_effective", "prefill_fused_projection_max_m", "profiled_total_gpu_ms", "component_sum_ms", "profiled_tokps_total", "component_ms", "component_share", "top_components", "layer_total_ms", "top_layers_by_total", "layer_top_components", "max_abs_diff_vs_native_prefill", "greedy_match_vs_native_prefill", "peak_vram_mb"])
        ...
        "state_scan_output_micro": [
            compact(r, ["_lineno", "status", "device", "dtype", "batch_size", "seq_len", "heads", "head_dim", "tokens_total", "bench_case", "num_warps", "num_stages", "triton_ms", "tokps_total", "component_ms", "delta_ms", "nokv_out_max_abs_diff", "nokv_state_max_abs_diff", "sk_out_max_abs_diff", "sk_state_max_abs_diff", "raw_output_max_abs_diff", "sk_output_max_abs_diff", "correction_output_max_abs_diff"])
            for r in state_scan_output_micro
        ],
        ...
        "dplr_compact_wy_stage_proto": [
            compact(r, ["_lineno", "status", "dtype", "device", "algorithm", "algorithm_family", "stage", "chunk_size", "compact_output_only", "compact_recompute_starts", "compact_prefix_shared", "compact_prefix_shared_group_size", "triton_compact_available", "triton_compact_summary_available", "triton_compact_prefix_available", "triton_compact_block_n", "triton_compact_block_r", "triton_compact_prefix_block_m", "triton_compact_start_state_dtype", "triton_apply_block_m", "B", "T", "H", "N", "warmup", "steps", "summary_shape", "summary_rank", "start_states_shape", "start_states_dtype", "chunk_end_shape", "state_source", "ms", "tokps", "recompute_prefix_probe_ms", "recompute_token_apply_probe_ms", "recompute_probe_sum_ms", "recompute_prefix_full_ratio", "recompute_token_apply_full_ratio", "recompute_probe_sum_full_ratio", "recompute_prefix_probe_shape", "recompute_prefix_probe_checksum_abs_max", "recompute_token_probe_out_shape", "prefix_shared_serial_chunks", "prefix_shared_loses_chunk_parallelism", "prefix_shared_groups", "transition_diag_max_abs_diff", "transition_left_max_abs_diff", "transition_right_max_abs_diff", "additive_left_max_abs_diff", "additive_right_max_abs_diff", "start_states_max_abs_diff", "prefix_vs_torch_summary_state_max_abs_diff", "chunk_end_max_abs_diff", "out_vs_apply_max_abs_diff", "out_max_abs_diff", "state_max_abs_diff", "out_min_cosine", "skip_reason", "error", "peak_vram_mb"])
            for r in dplr_compact_wy_stage_proto
        ],
...
def print_text(report: dict[str, Any]) -> None:
    ...
    if report["triton_state_scan_micro"]:
        ...
        print("PENDING")
    print("\n## state_scan_output_micro")
    if report["state_scan_output_micro"]:
        for row in report["state_scan_output_micro"]:
            print(json.dumps(row, ensure_ascii=False))
    else:
        print("PENDING")
    print("\n## decode_micro")
    ...
    if report["dplr_dense3_stage_proto"]:
        ...
        print("PENDING")
    print("\n## dplr_compact_wy_stage_proto")
    if report["dplr_compact_wy_stage_proto"]:
        for row in report["dplr_compact_wy_stage_proto"]:
            print(json.dumps(row, ensure_ascii=False))
    else:
        print("PENDING")
