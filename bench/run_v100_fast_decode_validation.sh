...
#   NATIVE_GRAPH_FUSED_OUTPUT_STEPS, NATIVE_GRAPH_FUSED_OUTPUT_BATCH_SIZES,
...

{
  ...
  run python bench/bench_fused_wavg_lora.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
  ...
  run python bench/bench_fused_recurrent_output.py \
    --hf-dir "${HF_DIR}" \
    --dtype "${DTYPE}" \
    --device "${DEVICE}" \
    --attn-mode fused_recurrent \
