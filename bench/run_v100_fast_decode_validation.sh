...
#   COMPONENT_STEPS, FUSED_PROJECTION_STEPS, FUSED_WA_LORA_STEPS, FUSED_SHIFT_MIX_STEPS,
#   FUSED_RECURRENT_STEPS, NATIVE_GRAPH_FUSED_RECURRENT_STEPS,
...

{
  ...
  echo "hf_dir=${HF_DIR}"
  echo "pth=${PTH}"
  echo "dtype=${DTYPE} device=${DEVICE} prompt_tokens=${PROMPT_TOKENS} decode_tokens=${DECODE_TOKENS} micro_steps=${MICRO_STEPS} forward_fast_steps=${FORWARD_FAST_STEPS} generate_batch_size=${GENERATE_BATCH_SIZE} generate_new_tokens=${GENERATE_NEW_TOKENS} warmup_batch_sizes=${WARMUP_BATCH_SIZES} native_graph_cache_size=${NATIVE_GRAPH_CACHE_SIZE} native_graph_overhead_batch_sizes=${NATIVE_GRAPH_OVERHEAD_BATCH_SIZES} native_graph_overhead_steps=${NATIVE_GRAPH_OVERHEAD_STEPS} component_steps=${COMPONENT_STEPS} fused_projection_steps=${FUSED_PROJECTION_STEPS} fused_wa_lora_steps=${FUSED_WA_LORA_STEPS} fused_shift_mix_steps=${FUSED_SHIFT_MIX_STEPS} fused_recurrent_steps=${FUSED_RECURRENT_STEPS} native_graph_fused_recurrent_steps=${NATIVE_GRAPH_FUSED_RECURRENT_STEPS} native_quant_gemv_steps=${NATIVE_QUANT_GEMV_STEPS} native_quant_w4_gemv_steps=${NATIVE_QUANT_W4_GEMV_STEPS} native_quant_rkv_steps=${NATIVE_QUANT_RKV_STEPS} native_quant_w4_rkv_steps=${NATIVE_QUANT_W4_RKV_STEPS}"
